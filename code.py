...
def categorical(df):
    ...
    print('Names of the categoriacal column are:',categorical_var.columns)
...
def numerical(df):
    numerical_var = df.select_dtypes('number')
    print('names of numerical columns:',numerical_var.columns)
...
def group_values(df,col1,agg1):
    grouping = df.groupby(col1).aggregate(agg1)
    return grouping
...
def convert(df,celsius):
    ...
    converted_temp= df[celsius].apply(lambda x:((x/5)*9)+32)
...
wind_speed_35_vis_25= instances_based_condition(weather,'Wind Spd (km/h)',35,'Visibility (km)',25.0)
...
weather['Temp (C)']= convert(weather,'Temp (C)')
weather['Dew Point Temp (C)'] = convert(weather,'Dew Point Temp (C)')
